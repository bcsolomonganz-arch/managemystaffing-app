...
// early arrivals.
function classifyPunch(emp, date, inTime, outTime, shifts, opts = {}) {
  ...
  const lateGrace  = opts.lateGraceMin  ?? 7;
  const earlyGrace = opts.earlyGraceMin ?? 15;
  if (schedIn != null && inMin != null && (inMin - schedIn) > lateGrace) return 'late';
  if (schedIn != null && inMin != null && (schedIn - inMin) > earlyGrace) return 'early';
  return 'normal';
}
...
function applyPunchEdit(emp, r, inTime, outTime, data) {
  ...
  if (r.status !== 'error') {
    r.status = classifyPunch(emp, r.date, r.in, r.out, data.shifts || []);
  }
